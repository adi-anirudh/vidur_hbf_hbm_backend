...
- Modeled expert storage: `2` bytes/parameter; one routed expert is `17.302` MB.
- HBM/HBF analytical bandwidth: `1024` GB/s.
- Analytical compute: `1000` TFLOP/s.
- At this roofline, one expert read costs the same time as approximately `976.6` expert-token computations.
...
The largest mean modeled reduction is `18.91%` at microbatch size `8` with `16` MoE GPUs. Interpret this only as a trace-driven scheduling result under the assumptions above.
