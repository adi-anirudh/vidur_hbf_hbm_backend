...
def main() -> None:
    ...
    axes[0].axhline(100, color="#555", ls=":", lw=1)
    axes[0].set_xticks(x)
    ...
    axes[0].set_ylim(max(0, min(qvals) - 5), max(qvals) + 5)
    axes[0].set_ylabel("Quality retained vs Full (%)")
    axes[0].set_title("(a) Same 10% token budget: quality")
    ...
    axes[1].set_xticklabels(clabel)
    axes[1].set_xlabel("Context length")
    axes[1].set_ylabel("Throughput/GPU (normalized to H3)")
    axes[1].set_title("(b) Same 10% token budget: performance")
    ...
    axes[2].set_xticklabels(clabel)
    axes[2].set_xlabel("Context length")
    axes[2].set_ylabel("Attention critical path (ms/token)")
    axes[2].set_title("(c) Attention-level latency")
    ...
    axes[3].set_xticks(xx)
    axes[3].set_xticklabels(labels)
    axes[3].set_ylabel("Internal HBF traffic (% of cold K+V)")
    axes[3].set_title("(d) Why page granularity is faster")
    axes[3].legend(frameon=False)
    ps.finish_axis(axes[3], grid_axis="y")
